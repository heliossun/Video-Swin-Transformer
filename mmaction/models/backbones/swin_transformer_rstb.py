...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            # flatten:B, C, D*Wh*Ww
            # transpose:B, D*Wh*Ww, C
            x = x.flatten(2).transpose(1, 2)
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.embed_dim, D, Wh, Ww)  # 拉回 B, C, D, Wh, Ww
...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # print('>>>>>>RSTB output shape=', (self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size), x_size))) + x).shape) # [4,65536,64]
